...
Try {
    ...
    [string]$appName = 'Microstation'
    [string]$appVersion = '2023'
    [string]$appArch = ''
    [string]$appLang = 'EN'
    [string]$appRevision = '01'
    [string]$appScriptVersion = '1.0.0'
    [string]$appScriptDate = '07/24/2023'
    ...
    If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
        ...
        Show-InstallationWelcome -CloseApps 'microstation,Bentley.Connect.Client,Bentley.Licensing.Service' -CheckDiskSpace -PersistPrompt
        ...
        $exitCode = Execute-Process -Path "$dirFiles\Setup_MicroStationx64_23.00.00.108.exe" -WindowStyle "Hidden" -WaitForMsiExec:$true -PassThru
        ...
        # The following code is for limiting the actions of the Connect Client, Bentley's user login application that will pop up and nag the many users who will not be using Microstation.
        # It sets registry keys to limit automatic login. See https://communities.bentley.com/communities/other_communities/licensing_cloud_and_web_services/w/wiki/36379/installation---silent

        Set-RegistryKey -Key 'HKEY_LOCAL_MACHINE\SOFTWARE\Bentley\BentleyDesktopClient\Install' -Name 'AutoLogin' -Value 'False'-Type String -ContinueOnError:$True
        Set-RegistryKey -Key 'HKEY_LOCAL_MACHINE\SOFTWARE\Bentley\BentleyDesktopClient\Install' -Name 'MinimizeOnSignIn' -Value 'True'-Type String -ContinueOnError:$True
        Set-RegistryKey -Key 'HKEY_LOCAL_MACHINE\SOFTWARE\Bentley\BentleyDesktopClient\Install' -Name 'ShowDesktopNotifications' -Value 'False'-Type String -ContinueOnError:$True
        Set-RegistryKey -Key 'HKEY_LOCAL_MACHINE\SOFTWARE\Bentley\BentleyDesktopClient\Install' -Name 'OnboardingShown' -Value 'True'-Type String -ContinueOnError:$True

        # The following is a premade script from https://www.hull1.com/scriptit/2022/11/18/noauto-bentley-connection-client.html that handles the automatic auto-run settings and
        # sets a schedule task to clean it up.
                ...
                #Everytime Connection Client is used it remakes the reg keys to auto start at login. Create a scheduled task to delete the reg keys every morning and night, at login, and at startup.
                $user = "SYSTEM"
                ...
                $trigger = @(
                    $(New-ScheduledTaskTrigger -Daily -DaysInterval 1 -At 6am),
                    $(New-ScheduledTaskTrigger -Daily -DaysInterval 1 -At 8pm),
    ...
    ElseIf ($deploymentType -ieq 'Uninstall') {
        ...
        Show-InstallationWelcome -CloseApps 'microstation,Bentley.Connect.Client,Bentley.Licensing.Service'
        ...
        $exitCode = Execute-Process -Path "$dirFiles\Setup_MicroStationx64_23.00.00.108.exe" -Parameters "-Uninstall -Quiet" -WindowStyle "Hidden" -WaitForMsiExec:$true -PassThru
        If (($exitCode.ExitCode -ne "0") -and ($mainExitCode -ne "3010")) { $mainExitCode = $exitCode.ExitCode }

        $exitCode = Execute-Process -Path "$dirFiles\Setup_CONNECTIONClientx64_23.00.00.10.exe" -Parameters "-Uninstall -Quiet" -WindowStyle "Hidden" -WaitForMsiExec:$true -PassThru
        If (($exitCode.ExitCode -ne "0") -and ($mainExitCode -ne "3010")) { $mainExitCode = $exitCode.ExitCode }
    ...
    ElseIf ($deploymentType -ieq 'Repair') {
        ...
        ## Show Welcome Message, close Internet Explorer with a 60 second countdown before automatically closing
        Show-InstallationWelcome -CloseApps 'processName'
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBcBvfm++B3JZFL
# p2+WBjH9W7Kqqgm9GdgmHtVm5ZudMKCCH8AwggVvMIIEV6ADAgECAhBI/JO0YFWU
...
# gjcCAQsxDjAMBgorBgEEAYI3AgEVMC8GCSqGSIb3DQEJBDEiB